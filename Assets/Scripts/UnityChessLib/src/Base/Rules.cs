...
namespace UnityChess {
    ...
    public static class Rules
    {
        ...
        {
            Debug.WriteLine($"[DEBUG] Checking if {player} is checkmated.");
            Square kingSquare = board.GetKingSquare(player);
            Debug.WriteLine($"[DEBUG] {player}'s King Square: {kingSquare}, legal moves : {numLegalMoves}");


            // 🔥 TEMPORARY DEBUGGING: Force checkmate by setting legal moves to 0
            if (numLegalMoves == 1)
            {
                Debug.WriteLine("[DEBUG] Forcing legalMoves to 0 for testing checkmate detection.");
                numLegalMoves = 0;
            }
            ;

            return numLegalMoves <= 0 && IsPlayerInCheck(board, player);
        }
        ...
        {
            Debug.WriteLine($"[DEBUG] Checking if {player} is stalemated.");
            Square kingSquare = board.GetKingSquare(player);
            Debug.WriteLine($"[DEBUG] {player}'s King Square: {kingSquare}, legal moves : {numLegalMoves}");
            //// 🔥 TEMPORARY DEBUGGING: Force checkmate by setting legal moves to 0
            //if (numLegalMoves == 1)
        ...
        {
            Debug.WriteLine($"[DEBUG] Checking if {player} is in check.");
            Square kingSquare = board.GetKingSquare(player);
            Debug.WriteLine($"[DEBUG] {player}'s King Square: {kingSquare}");

            return IsSquareAttacked(kingSquare, board, player);
        }
